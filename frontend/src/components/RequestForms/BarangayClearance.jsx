...
export default function BarangayClearance() {
  ...
  // In the BarangayClearance component, add these state variables
...
const [validCertInfo, setValidCertInfo] = useState(null);
  ...
  async function loadRecords() {
    try {
      const res = await fetch(`${apiBase}/barangay-clearance`, { headers: getAuthHeaders() });
      const data = await res.json();
      setRecords(
        Array.isArray(data)
          ? data.map((r) => ({
  ...
  async function handleUpdate() {
    try {
      ...
      const updatedRec = { 
        ...updatedData,
        validity_period: getValidityPeriod('Barangay Clearance')
      };
      // Remove old record and add new one
      setRecords([updatedRec, ...records.filter((r) => r.barangay_clearance_id !== editingId)]);
...
>
  ...
  <DialogContent sx={{ p: 3 }}>
    <Typography>
      This resident already has a valid Barangay Clearance issued on{' '}
