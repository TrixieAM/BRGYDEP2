...
export default function PermitToTravel() {
  ...
  function storeCertificateData(certificateData) {
    ...
    existingCertificates[certificateData.permit_to_travel_id] = certificateData;
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      if (display.permit_to_travel_id || display.full_name) {
  ...
  async function handleDelete(id) {
    ...
    try {
      const res = await fetch(`${apiBase}/permit-to-travel/${id}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
      if (!res.ok) throw new Error('Delete failed');
      setRecords(records.filter((r) => r.permit_to_travel_id !== id));
      if (selectedRecord?.permit_to_travel_id === id) setSelectedRecord(null);
  ...
  async function generatePDF() {
    ...
    try {
      ...
      const fileName = `Permit_to_Travel_${display.permit_to_travel_id}_${display.full_name.replace(/\s+/g, '_')}.pdf`;
      pdf.save(fileName);
    } catch (error) {
  ...
  const handleQrCodeClick = () => {
    if (display.permit_to_travel_id) {
      ...
      const verificationUrl = `${window.location.origin}/verify-certificate?id=${display.permit_to_travel_id}`;
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ opacity: 0.9 }}>
                  Manage all records of the Permit to Travel
                </Typography>
              </Box>
            </Box>
                ...
                <Box sx={{ display: "flex", gap: 1 }}>
                  <Tooltip title="Download PDF">
                    <Button 
                      ...
                      color="secondary" 
                      onClick={generatePDF} 
                      disabled={!display.permit_to_travel_id || isGeneratingPDF} 
                  ...
                  <Tooltip title="Print">
                    <Button 
                      variant="outlined" 
                      onClick={handlePrint} 
                      disabled={!display.permit_to_travel_id}
                      startIcon={<PrintIcon />}
                      size="small"
                    >
                      Print
                ...
                >
                  ...
                  >
                    ...
                    <p style={{ margin: 0, textIndent: '50px' }}>
                      ...
                      <i> Permit to Travel</i> from this office and the result/s
              ...
              <Box sx={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                <Paper elevation={0} sx={{ p: 3, borderBottom: 1, borderColor: "divider" }}>
                  <Typography variant="h6" sx={{ fontWeight: 600, mb: 1, display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    {editingId ? "Edit Certificate" : "New Permit to Travel"}
                  </Typography>
                ...
                <Box sx={{ flex: 1, overflow: "auto", p: 3 }}>
                  <Stack spacing={3}>
                    ...
                    <TextField 
                      ...
                      multiline 
                      rows={2}
                      placeholder="Travel abroad, Vacation, etc."
                  ...
                  {filteredRecords.length === 0 ? (
                    ...
                    <Stack spacing={2}>
                      {filteredRecords.map((record) => (
                        <Card key={record.permit_to_travel_id} sx={{ 
                          cursor: "pointer",
                            ...
                            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                              ...
                              <Box sx={{ display: "flex", gap: 0.5 }}>
                                ...
                                <Tooltip title="Delete">
                                  <IconButton 
                                    size="small" 
                                    onClick={() => handleDelete(record.permit_to_travel_id)} 
                                    color="error"
                                  >
                                    <DeleteIcon />
                                  </IconButton>
        ...
        <DialogContent dividers>
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              ...
              >
                {display.permit_to_travel_id || 'Draft (Not yet saved)'}
              </Typography>
            </Grid>
            <Grid item xs={12} md={6}>
              <Typography variant="body2" sx={{ color: 'grey.600' }}>
