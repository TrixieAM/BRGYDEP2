...
router.get('/', async (req, res) => {
  try {
    ...
    const [columns] = await pool.query("SHOW COLUMNS FROM cash_assistance");
    ...
    if (!hasUseSignature) {
      await pool.query("ALTER TABLE cash_assistance ADD COLUMN use_signature TINYINT(1) DEFAULT 0");
    }
    if (!hasSecretarySignatureId) {
      await pool.query("ALTER TABLE cash_assistance ADD COLUMN secretary_signature_id INT NULL");
      // Add foreign key if official_signature table exists
      try {
        await pool.query("ALTER TABLE cash_assistance ADD FOREIGN KEY (secretary_signature_id) REFERENCES official_signature(signature_id)");
      } catch (err) {
    ...
    if (!hasCaptainSignatureId) {
      await pool.query("ALTER TABLE cash_assistance ADD COLUMN captain_signature_id INT NULL");
      // Add foreign key if official_signature table exists
      try {
        await pool.query("ALTER TABLE cash_assistance ADD FOREIGN KEY (captain_signature_id) REFERENCES official_signature(signature_id)");
      } catch (err) {
        // Foreign key might already exist or official_signature table might not exist
    ...
    const [rows] = await pool.query(`
      ...
      FROM cash_assistance ca
      LEFT JOIN official_signature sec_sig ON ca.secretary_signature_id = sec_sig.signature_id
      LEFT JOIN official_signature cap_sig ON ca.captain_signature_id = cap_sig.signature_id
...
router.get('/transactions/all', async (req, res) => {
  try {
    const [rows] = await pool.query(`
      ...
      LEFT JOIN official_signature sec_sig ON ca.secretary_signature_id = sec_sig.signature_id
      LEFT JOIN official_signature cap_sig ON ca.captain_signature_id = cap_sig.signature_id
      ORDER BY ca.date_created DESC, ca.cash_assistance_id DESC
    `);
    res.json(rows);
  } catch (err) {
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // Check if the table has the required columns
    const [columns] = await connection.query("SHOW COLUMNS FROM cash_assistance");
    const hasUseSignature = columns.some(col => col.Field === 'use_signature');
    const hasSecretarySignatureId = columns.some(col => col.Field === 'secretary_signature_id');
    const hasCaptainSignatureId = columns.some(col => col.Field === 'captain_signature_id');
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const newTransactionNumber = generateTransactionNumberForType('CA');
    ...
    await connection.rollback();
    console.error('Error updating cash assistance record:', err);
